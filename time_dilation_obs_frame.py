...
class light_clock:
    ...
    def __init__(self, loc):
        ...
        # clock parts        
        self.bottom = box(pos=loc+vector(0,-self.w/2,0),
                     size=vector(self.L,s,self.w),
                     color = color.blue)
        
        self.top = box(pos=loc+vector(0,+self.w/2,0),
                  size=vector(self.L,s,self.w),
                  color = color.blue)
...
obs = observer(vector(1.5,-2,0))
clock1 = light_clock(vector(0,1,0))
clock2 = light_clock(vector(0,-1,0))
clock3 = light_clock(vector(3,-1,0))
