...
carriage = car(vector(0,0,0))
bulb = sphere(pos=vector(+0.5,-1.0,0),radius= 0.1,color=color.magenta)
flash1 = sphere(pos=bulb.pos,radius= 0.05,color=color.yellow)
flash2 = sphere(pos=bulb.pos,radius= 0.05,color=color.yellow)
...
while t < 10000:
    ...
    bulb.pos += vector(-v*t,0,0)
    flash1.pos += vector(-v*t,0,0)
    flash2.pos += vector(-v*t,0,0)
    
    if bulb.pos.x - carriage.facing.pos.x < 0.0001:
        
        flash1.pos += vector(+5.0*v*t,0,0)    
        flash2.pos += vector(-3.0*v*t,0,0)    
        
        posgraph1.plot(pos=(t, abs(flash1.pos.x)))
        posgraph2.plot(pos=(t, abs(flash2.pos.x)))
